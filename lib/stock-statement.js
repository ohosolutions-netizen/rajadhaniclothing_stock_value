...
async function getAccessToken() {
  ...
  if (!refreshToken || !clientId || !clientSecret) {
    if (process.env.ZOHO_ANALYTICS_ACCESS_TOKEN?.trim()) {
      return process.env.ZOHO_ANALYTICS_ACCESS_TOKEN.trim();
    }
  ...
  if (!response.ok || !result.access_token) {
    throw new Error(`Zoho OAuth token refresh failed: ${result.error || "unknown_error"}`);
  }
